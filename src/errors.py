...
class Error:
    ...
    def port_error(self, port):
        ...
        try:
            ...
            - If you have a slower internet connection, changing the value of cooldown located in config.json 
              to 0 or any number greater than 1 may help.
    ...
    def lockfile_error(self, path, ignore_lockfile=False):
        # ignoring lockfile is for when lockfile exists,
        # but it's not really valid, (local endpoints are not initialized yet)
        if os.path.exists(path) and ignore_lockfile is False:
            return True
        else:
