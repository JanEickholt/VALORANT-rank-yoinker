            ...
            try:
                if presence["isValid"]:
                    if presence["sessionLoopState"] == "INGAME":
                        ...
                        map_text = self.map_dict.get(presence["matchMap"].lower())
                        if map_text == "The Range":
                            map_image = "splash_range_square"
                            details = "in Range"
                            agent_img = str(self.data.get("rank"))
                            agent = self.data.get("rank_name")
                        else:
                            map_image = f"splash_{self.map_dict.get(presence['matchMap'].lower())}_square".lower()
                        if map_text is None or map_text == "":
                            map_text = None
                            map_image = None
                        ...
                        if (presence["provisioningFlow"] == "CustomGame" or
                                presence["partyState"] == "CUSTOM_GAME_SETUP"):
                            gamemode = "Custom Game"
                        else:
                            gamemode = self.gamemodes.get(presence['queueId'])
